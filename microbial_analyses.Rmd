...
microbe_reports<-read.csv("~/Downloads/microbial_data_bg_corrected.csv", header=TRUE) 
...
map<-read.csv("~/Downloads/patient_ID_sample_name_key.csv", header=TRUE)

### Read in metadata file and join
meta<-read.csv("~/IvC/metadata.csv", header=TRUE)
metadata<-meta %>% 
  mutate(age=X_AGE_YEARS,gender=X_SEX, category_infection_class=subgroup, infection_derived=group) %>% left_join(map, by="Patient")
...
load_data <- my_data %>%
  ...
  filter(grepl("Bacterial|CTRL",category_infection_class)) %>%
  mutate(category_derived=case_when(
    ...
    grepl("Bacterial IPC", category_infection_class) ~ "Bacterial\nIPC", 
    grepl("Bacterial/viral IPC", category_infection_class) ~ "Bacterial/viral\nIPC", 
    grepl("CTRL", category_infection_class, ignore.case = TRUE) ~ "CTRL",
    TRUE ~ "NA")) %>% 
...
statsa <-load_data %>% 
  filter(grepl("CTRL|Bacterial",category_infection_class))%>%
...
trimmed<-load_data %>% filter(grepl("CTRL|Bacterial",category_infection_class))
...
load_data <- my_data %>%
  ...
  filter(infection_derived!="NA") %>%
  filter(grepl("Viral|CTRL|viral",category_infection_class)) %>%
  mutate(category_derived=case_when(
    grepl("Viral LRTI", category_infection_class) ~ "Viral\nLRTI", 
    grepl("Bacterial/viral LRTI", category_infection_class) ~ "Bacterial/viral\nLRTI", 
    grepl("Viral IPC", category_infection_class) ~ "Viral\nIPC", 
    grepl("Bacterial/viral IPC", category_infection_class) ~ "Bacterial/viral\nIPC", 
    grepl("CTRL", category_infection_class, ignore.case = TRUE) ~ "CTRL",
    TRUE ~ "NA")) %>% 
  dplyr::mutate(sumlogrpm = ifelse(is.na(sumlogrpm), 0, sumlogrpm)) %>% # set sum dpm to 0 if it's NA
...
trimmed<-load_data %>% filter(grepl("Viral|CTRL|viral", category_infection_class))
statsa <-trimmed %>% 
  wilcox_test(data=.,sumlogrpm ~ category_derived) %>%
  add_significance("p") %>%
  add_xy_position(x = "Infection Status", dodge=0.4) %>% 
...
data7 <-data6 %>%
  mutate(infection_derived=case_when(grepl("LRTI", infection_derived) ~ "LRTI", grepl("IPC", infection_derived) ~ "IPC", grepl("CTRL", infection_derived) ~ "CTRL", TRUE ~ "NA"))
...
### read in data
rbm<-read.csv("~/Downloads/RBM_results.csv", header=TRUE)
viral<-read.csv("~/Downloads/Viral_pathogens.csv", header=TRUE)
knownresp<-read.csv("~/known_pathogens.csv", header=TRUE)
...
### get nonhost info ###
sample_overviews <-read.csv("~/CZID_data_summary.csv", header=TRUE)
so<-sample_overviews %>%
...
panele1 <- ggplot(input_data,
  ...
  #P-value annotation — now horizontal
...
metadata2 <-metadata %>% 
  select(sample_name, category_infection_class, gender, age, group) %>%
  mutate(infection_derived=group)
...
### Trim data
meta1<-metadata3 %>% filter(infection_derived=="LRTI" | infection_derived=="IPC")
...
ggplot(volcano_df, aes(x = logFC, y = neg_log10_padj, label = taxa)) +
  ...
  geom_text_repel(
    ...
    aes(label = taxa, color = sig),    # 👈 color segments by group
    max.overlaps = Inf,
    size = 3,
    box.padding = 0.3,
    show.legend = FALSE,
...
```{r read in data}
blast<-read.csv("~/VFDB_blast_results.csv",header=TRUE)
info<-read.table("~/IvC/VFDB/VF_info_file.tsv", sep="\t",header=TRUE)
...
### metadata ###
metadata2<-metadata %>%
  select(sample_name,gender,age,category_infection_class, group, Patient) 

### add in total ribosome-depleted reads from sample overviews file 
sample_overviews<-read.csv("~/CZID_data_summary.csv", header=TRUE) 
...
my_data<-results_filtered %>%
  ...
  left_join(metadata,by="sample_name") %>%
  filter(!is.na(category_infection_class)) %>%
  left_join(sample_overviews, by="sample_name")
### 254 samples ###
...
data_read_level_all<-my_data%>%
  ...
  mutate(sample_total_reads=n(),
         million_reads=total_reads_depleted/1000000,
  ...
  ungroup() %>%
  select(sample_name, VFid, VF, category, gene_length, total_reads, total_reads_depleted,million_reads, sample_total_reads, sample_total_rpm, sample_vfid_reads, sample_vfid_bases, sample_vfid_coverage, sample_vfid_rpm, sample_vfid_dpm, sample_category_reads, sample_category_rpm, category_infection_class, infection_derived) %>%
  distinct()

data_read_level<-my_data %>%
  mutate(gene_length=length.y, hit_length=length.x) %>%
  filter(grepl("LRTI|IPC", infection_derived)) %>%
  group_by(sample) %>%
  mutate(sample_total_reads=n(),
         million_reads=total_reads_depleted/1000000,
         sample_total_rpm=sample_total_reads/million_reads) %>%
...
### ID top 50 VFs
top50<-data_read_level %>% 
  select(VFid, VF, sample_vfid_dpm) %>%
  distinct() %>%
  group_by(VF) %>%
  summarise(total_DPM = sum(sample_vfid_dpm), na.rm = TRUE) %>%
  arrange(desc(total_DPM)) %>%
  slice_head(n = 50)

# Build data #
collapsed<-data_read_level %>%
  select(sample_name, VF, VFid, sample_vfid_reads, sample_vfid_rpm, sample_vfid_dpm, infection_derived) %>%
  group_by(sample_name,VF) %>%
  mutate(sample_vf_reads=sum(sample_vfid_reads), 
         sample_vf_rpm=sum(sample_vfid_rpm), 
         sample_vf_dpm=sum(sample_vfid_dpm)) %>%
  ungroup() %>%
  select(sample_name, VF, sample_vf_reads, sample_vf_rpm, sample_vf_dpm, infection_derived) %>%
  distinct() %>%
  filter(VF %in% top50$VF)

metadata$age <-as.integer(metadata$age)

### Create and format metadata
metadata2 <-metadata %>% select(sample_name, category_infection_class, gender, age, infection_derived) 
rownames(metadata2) <- metadata2[,1]
metadata3 <- metadata2 %>%
  mutate(infection_derived = factor(infection_derived, c( "CTRL", "IPC", "LRTI")))
...
counts<-collapsed %>% 
  ...
  pivot_wider(names_from = VF, id_cols = sample_name, values_from = sample_vf_reads, values_fill=0)
